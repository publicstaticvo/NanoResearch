...
class _LaTeXBibFiguresMixin:
    ...
    def _validate_figures_in_latex(
        ...
        Failed/empty figures get a placeholder block when needed so references
        do not become dangling.
        ...
        failed_figures: list[tuple[str, str, str]] = []
        for fig_key, fig_data in figures.items():
            ...
            is_failed = fig_data.get("status") == "failed" or (
                "error" in fig_data and "png_path" not in fig_data and "pdf_path" not in fig_data
            )
            if is_failed:
                if f"\\label{{fig:{label_suffix}}}" not in latex_content:
                    ...
                    self.log(f"  VALIDATION: failed figure '{fig_key}' already has label")
                continue
