...
def render_router_hindsight_context(
    ...
    *,
    ...
    """Render policy-level guidance for same-router hindsight decisions."""
    if not enabled:
        return ""

    profile = profile if isinstance(profile, dict) else {}
    task_type = task_type.lower().strip()
    router_hints = profile.get("router_hints", {})
    ...
    if task_type in {"ideation", "planning", "literature"}:
        lines.extend([
            ...
            "Reuse memory and skills when they sharpen the plan; avoid extra retrieval or tool use when the profile already gives a clear bias.",
        ])
    elif task_type in {"experiment", "execution", "analysis", "review"}:
        lines.extend([
            "Use execution or review feedback as hindsight for the next router decision.",
        ...
        lines.extend([
            "Keep the writing prompt aligned with the router's learned venue and claim policy.",
